...
public class apitesting {
	...
	public static void main(String[] args) {
		...
		Response createdResponse = RestAssured.given().auth().basic("evaritesh@com", "evaritesh@2001").contentType(ContentType.JSON).
				accept(ContentType.JSON).body(bodyContact).post("https://evaritesh.agilecrm.com/dev/api/contacts/");
		String createdContact = createdResponse.getBody().asPrettyString();
		System.out.println(createdContact);

		JSONObject rootJsonObj = new JSONObject(createdContact);
		JSONArray propertyArray=rootJsonObj.getJSONArray("properties");
		JSONObject propArrayFirst=propertyArray.getJSONObject(0);
		String value=propArrayFirst.getString("value");
